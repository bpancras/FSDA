function [out]  = MixSimCodeGen(k,v,options)%#codegen
...
if nargin > 2
    ...
    checkBarOmega = isprop(options, 'BarOmega');
    checkMaxOmega = isprop(options, 'MaxOmega');
...
%% Beginning of inner functions
    ...
    function  Q = OmegaClust(Omega, method, v, k, PiLow, Lbound, Ubound, ...
        ...
        for isamp=1:resN
            ...
            if islogical(sph) && nargin>13 && ~isempty(restrfactor)
                U=complex(initzeros(v,v,k));
                S05=Sgen;
                Sinv=Sgen;
                detS=initzeros(k,1);
                Lambda_vk=complex(initzeros(v,k));
                for j=1:k
            ...
            elseif method==1.5
                ...
                %                     overlapc=[overlapv; initzeros(combk2-length(overlapv),1)];
                %                 else
                %                     overlapc=overlapv;
                %                 end
        ...
        %             overlapc=[overlapv; initzeros(0.5*k*(k-1)-length(overlapv),1)];
        %         else
        %             overlapc=overlapv;
        %         end
        %         % Compute standard deviation of overlap for current
    ...
    function Q=OmegaBarOmegaStd(p, k, PiLow, Lbound, Ubound, ...
        ...
        for isamp=1:resN
            ...
            if diff < -tolmap % Requested StdOmega of overlapping is reachable
                ...
                disp(['In simulation ' sprintf('%d', int64(isamp))])
                disp(['MaxOmega(achievable)=' sprintf('%d', int64(Malphaini)) ' and'])
    ...
    function  [c, OmegaMap2, BarOmega2, MaxOmega2, rcMax]=FindC(lower, upper, Omega, method, v, k, li, di, const1, fix, tol, lim)
        ...
        %          or deflation. If fix=initzeros(k,1) all clusters participate
    ...
    function o = num2str(x)
        o = sprintf('%d', int64(x));
    end

    function o = initzeros(varargin)
        o = complex(zeros([varargin{:}]));
    end
