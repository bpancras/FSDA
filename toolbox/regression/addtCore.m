function out = addtCore(y, X, w, options)
...
if coder.target('MATLAB')
    if (options.plots==1)
        if ~isempty(options.units)
            sel=setdiffFS(1:length(y),options.units);
            [outsel]=addt(y(sel),X(sel,2:end),w(sel),'plots',0);

            plot(Aw(sel),Az(sel),'+b','MarkerSize',options.FontSize);
            hold('on');
            plot(Aw(options.units),Az(options.units),'or','MarkerSize',6,'MarkerFaceColor','r');
            ...
            if (options.textlab==true)
                text(Aw(options.units)+0.05,Az(options.units),num2str(options.units),'FontSize',options.FontSize);
            end
            set(gca,'FontSize',options.SizeAxesNum)

            xlabel('Aw','Fontsize',options.FontSize);
            ylabel('Ay','Fontsize',options.FontSize);
